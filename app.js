...
let st={
  ...
  pickPerformance:[],pickPerformanceSnaps:[],statsTimeWindow:"last_30d",statsLeaderMetric:"H",leaderMode:"final",leaderDateOffset:0,gameEntry:{selectedGame:null,legCount:GAME_ENTRY_DEFAULT_LEGS,entry:null},
...
function getPickGuard(latestDate,picksCount){
  const tonightCount=st.mode==="pitcher"?st.pTonight.length:st.tonight.length;
...
function renderTonightShortlist(){
  ...
  return`<section class="shortlist-shell"><div class="shortlist-head"><div><div class="analysis-eyebrow">Tonight's decision board</div><div class="shortlist-title">Tonight's Shortlist</div><div class="shortlist-sub">Qualified opportunities only: +5% modeled edge, playable price, 5+ decisive games, no sample warning, no lineup risk, no model disagreement, and not locked.</div></div><div class="shortlist-rule">Ranked by conviction</div></div><div class="shortlist-kpis"><div class="shortlist-kpi"><strong>${rows.length}</strong><span>Qualified</span></div><div class="shortlist-kpi"><strong>+${(avgEdge*100).toFixed(1)}%</strong><span>Average edge</span></div><div class="shortlist-kpi"><strong>${aiBacked}</strong><span>AI backed</span></div><div class="shortlist-kpi"><strong>${markets}</strong><span>Markets</span></div></div>${renderShortlistTray()}<div class="shortlist-grid">${rows.map((row,index)=>{
    ...
    const gameTime=gameStartTimeForTeams(row.team,row.opp)||gameStartTimeForText(row.game||"");
...
function streakToDash(name,metric,line){
  ...
  if(pitcherMetric){
    ...
    st.mode="batter";st.metric=metric||"H";st.player=b.player_name;
  }else if(p){
    st.mode="pitcher";st.metric=metric||"SO";st.player=p.player_name;
  }
...
function metricPrefersUnder(metric,{isPitcher=false}={}){
  const normalized=String(metric||"").toUpperCase();
  return !!isPitcher&&["ER","BB","H","P_ER","P_BB","P_H"].includes(normalized);
}
...
function renderFeaturedPick(model){
  const p=model.pk,status=pickStatusLine(model),evidence=pickEvidenceHTML(model),gameTime=gameStartTimeForText(p.game)||gameStartTimeForTeams(p.team_abbr,p.opp_abbr_tonight);
...
function renderDashboardPage(){
  ...
  const analysisLean=pickMatchesMetric?normalizeLeanText(pPick.lean):(metricPrefersUnder(metric,{isPitcher:isP})?"UNDER":"OVER");
